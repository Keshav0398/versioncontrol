...
export default class productSelectorBarcode extends LightningElement {
    @track searchTerm = '';
    ...
    handleBarcodeScan(event) {
        ...
        if (this.searchTerm && this.searchTerm.length > 7) {
    ...
    handleCreateBill() {
        if (this.jsPDFInitialized) {
            const productsPerPage = 12; // Limit products per page (or adjust as needed)
            const productCount = this.selectedItems.length;
            const additionalPages = Math.ceil((productCount - productsPerPage) / productsPerPage);
            
            // Calculate dynamic page height (e.g., 70mm base height + 20mm per additional page)
            const pageHeight = 100 + additionalPages * 20;
    
            const doc = new window.jspdf.jsPDF({
                unit: 'mm',
                format: [70, pageHeight] // Adjust height based on product count
            });
            ...
            const pageHeightLimit = doc.internal.pageSize.getHeight();
            if (yOffset + 20 < pageHeightLimit) {
                yOffset = pageHeightLimit - 10; // Move yOffset to near bottom of the page if space permits
            } else {
                yOffset += 12; // Otherwise, just move it a little further down after totals
            }
    
            // Contact info at the bottom
            doc.setFontSize(7);
            const pageHeightx = doc.internal.pageSize.getHeight();
            doc.text('Contact: 7869398623    7509596165', 5, pageHeightx - 10);
